...
function page() {
  ...
  useEffect(() => {
    fetch(`${NEXT_PUBLIC_BACKEND_URL}/case-studies-find/${id}`)
      .then((res) => res.json())
      .then((data) => {
        setTitle(data.title);
        setSummary(data.summary);
        setCategory(data.category);
        setImageData(data.coverImage);
        setProjectGoal1(data.goal1);
        setProjectGoal2(data.goal2);
        setProjectGoal3(data.goal3);
        setContent(data.content);
        setFeatureDesc(data.featureDesc);
        setStackDesc(data.stackDesc);
        setImages(data.screenshots);
        setStackImages(data.stackImages);
        setFeatures(data.features);
        setAltText({
          cover_image: data.altText.cover_image || "",
          text_image: data.altText.text_image || [],
          moreScreen_image: data.altText.moreScreen_image || [],
          stack_image: data.altText.stack_image || [],
        });
      })
      .catch((err) => {
  ...
  const handleSubmit = async (e) => {
    ...
    fetch(`${NEXT_PUBLIC_BACKEND_URL}/case-studies-edit/${id}`, {
      method: "PUT",
